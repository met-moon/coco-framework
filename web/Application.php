    ...
    public function dispatch()
    {
        if ($this->routeType == 1) {
            ...
            Route::error(function ($route) {
                ...
                throw new Exception('Page Not Found', "此路由：\"{$_SERVER['REQUEST_METHOD']}: $route\" 无匹配项");
            });
            Route::dispatch();
            $this->end();
        }
        ...
        if ($this->module == $this->defaultModule) {
            ...
            if (!empty($this->config['modules'][$this->module]['controllerNamespace'])) {
                $className = $this->config['modules'][$this->module]['controllerNamespace'] . '\\' . $this->controller . 'Controller';
            } else {
                $className = 'app\\modules\\' . $this->module . '\\controllers\\' . $this->controller . 'Controller';
            }
        }
